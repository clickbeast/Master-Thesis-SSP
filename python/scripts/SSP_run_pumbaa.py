...
root_path = "/home/simonvermeir/SSP"
...
class Runner():
    ...
    def run_parallel(self, jobs=jobs, background=background):
        if background:
            command = "nohup parallel --jobs {}  --progress < {} & echo $! > process.txt | echo".format(jobs, self.fp_commands)
        else:
    ...
    def create_jar_name(self, use_multi, use_static=False):
    ...
    def create_commands_files(self,
        ...
        os.chdir(root_path)
        jar_file = rdp_builds + self.create_jar_name(use_multi, use_static= not build_jar) + ".jar"
                ...
                if self.instance_allowed(file, self.sanitize_filter(filter)) or use_filter is False:
                    for multi_index in range(multi):
                        for run in range(runs):
                            ...
                            self.config["params"][multi_index]["project_root"]  = root_path
